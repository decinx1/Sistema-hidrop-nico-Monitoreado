...
class DayDialog(QDialog):
    def __init__(self, date, parent=None):
        ...
        self.setStyleSheet("""
            QDialog { background-color: white; border: 1px solid #e0e0e0; border-radius: 5px; }
...
class CalendarWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        grid_widget.setStyleSheet("QWidget { background-color: white; border-radius: 0 0 5px 5px; }")
    ...
    def create_header(self, main_layout):
        ...
        header.setStyleSheet("""
            QWidget { background-color: #80CC27; border-radius: 5px 5px 0 0; padding: 10px; }
            QLabel, QComboBox, QSpinBox { color: white; font-size: 14px; }
            QComboBox, QSpinBox {
                background-color: rgba(255,255,255,0.2);
                border: 1px solid rgba(255,255,255,0.3);
                border-radius: 3px; padding: 3px; min-width: 60px;
            }
        ...
        nav_btn_style = """
            ...
            QPushButton:hover { background-color: rgba(255,255,255,0.2); border-radius: 3px; }
        """
    ...
    def create_weekdays_header(self, main_layout):
        week_days = ["DOM", "LUN", "MAR", "MIÉ", "JUE", "VIE", "SÁB"]
        days_header = QWidget()
        days_header.setStyleSheet("""
            QWidget { background-color: #80CC27; }
            QLabel { font-weight: bold; color: white; padding: 8px 0; }
        """)
    ...
    def updateCalendar(self):
        ...
        for i in range(days_to_show):
            ...
            self.add_day_button(day, row, i, disabled=True)
        ...
        for day in range(1, days_in_month + 1):
            ...
            btn = self.add_day_button(day, row, col, not is_past and not is_today)
            
            if is_past or is_today:
                btn.clicked.connect(lambda _, d=current_date: self.showDayDetails(d))
                style = self.get_day_style(is_today, is_past)
        ...
        while row < 6 or (row == 6 and col < 7):
            self.add_day_button(next_month_day, row, col, disabled=True)
            next_month_day += 1
            col += 1
            if col > 6:
                col = 0
    ...
    def add_day_button(self, day, row, col, disabled=False):
        ...
        if disabled:
            btn.setStyleSheet("""
                QPushButton { color: #CCCCCC; font-size: 14px; background-color: white; }
            """)
        
        self.calendar_grid.addWidget(btn, row, col)
        return btn
    
    def get_day_style(self, is_today, is_past):
        if is_today:
            return """
                QPushButton {
                    font-size: 14px; font-weight: bold; color: white;
                    background-color: #FF5722; border: 1px solid #E64A19;
                }
            """
        else:
            return """
                ...
                QPushButton:hover {
                    background-color: #E1F5FE; color: #0088CC;
                    border: 1px solid #01ADEF;
                }
                QPushButton:pressed {
                    background-color: #B3E5FC; color: #006699;
                }
            """
